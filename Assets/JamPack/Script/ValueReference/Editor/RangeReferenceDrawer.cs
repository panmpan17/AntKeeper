...
namespace MPack
{
    ...
    public class RangeReferenceDrawer : RereferenceDrawer
    {
        ...
        {
            string path = EditorUtility.SaveFilePanelInProject("New Range Varible", string.Format("New {0}.asset", propertyName), "asset", "Test");

            if (path != "")
            {
                var newVarible = ScriptableObject.CreateInstance<RangeVariable>();
