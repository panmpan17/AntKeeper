...
namespace MPack
{
    ...
    public class IntReferenceDraer : RereferenceDrawer
    {
        ...
        {
            string path = EditorUtility.SaveFilePanelInProject("New Int Varible", string.Format("New {0}.asset", propertyName), "asset", "Test");

            if (path != "")
            {
                var newVarible = ScriptableObject.CreateInstance<IntVariable>();
